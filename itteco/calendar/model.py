...
class Calendar(object):
    ...
    def __init__(self, env, cal_id=None, owner=None, db=None):
        self.env = env
        self._exists = False
        self.type = CalendarType.Private
        self.ref=self.name= self.alias =self.theme= None
        self.owner= owner
        self.id=cal_id
        self.resource = Resource('calendar', cal_id)
    ...
    def _fetch(self, cal_id, owner, db=None):
        ...
        row = self._get_row(cal_id, db)
        if row:
            self._from_db_row(row)
        
    def _get_row(self, cal_id, db):
        cursor = db.cursor()
        cursor.execute("""
    ...
    def _from_db_row(self, row):
        ...
        self.resource = self.resource(id=cal_id)
        self.id = cal_id
        self.name =  name
        self.owner = owner
        self.type = type
    ...
    def select(env, **kwargs):
        ...
        cursor.execute("SELECT id, name, owner, type, alias, theme, ref, created, modified " +
            "FROM calendar "+ where_smt, tuple(kwargs.values()))
...
class Event(object):
    ...
    def _fetch(self, event_id, db=None):
        ...
        cursor = db.cursor()
        cursor.execute(
            """SELECT id, title, description, calendar_id, allday,
    ...
    def _from_db_row(self, row):
        types = [T, T, T, nvl_int, nvl_int, nvl_int, dt, dt, dt, dt]
        event_id, title, description, calendar_id, allday, ticket, dtstart, dtend, created, modified = \
    ...
    def insert(self, db=None):
        ...
        self.time_created = self.time_modified = datetime.now(utc)
        types = [T, T, int, int, int, to_timestamp, to_timestamp, to_timestamp, to_timestamp]
        self.env.log.debug("tts %s->%s %s->%s %s>%s %s>%s)" %
                            (
                                self.dtstart, type(self.dtstart),
                                self.dtend, type(self.dtend),
                                self.time_created, type(self.time_created),
                                self.time_modified, type(self.time_modified)
                            )
        )

        cursor = db.cursor()
        cursor.execute("INSERT INTO cal_event(title, description, calendar_id, allday, ticket, dtstart,"+
            " dtend, created, modified) VALUES (%s)" % ("%s,"*len(types))[:-1],
                       [t(v) for t, v in zip(
                            types, 
                            (
                                self.title, 
    ...
    def select(env, owner, daterange=None):
        ...
        sql = """
              SELECT ev.*, ev.id as event_id, tt.owner, tt.auto, tt.time
                FROM
                      (SELECT e.id as id, e.title, e.description, e.calendar_id, e.allday, e.ticket, 
                              e.dtstart as dtstart, e.dtend as dtend, e.created, e.modified
                         FROM all_cal_events as e
                              INNER JOIN calendar as c 
                                 ON e.calendar_id = c.id AND (c.owner=%s OR c.type='S')) ev
