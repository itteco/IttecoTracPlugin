		...
		var serializeToXml = function(data) {
			...
			case 'object':
				if(data instanceof Date) {
                    ...
                    var dt = new Date(+data+(offset * 60 * 1000));
					return '<dateTime.iso8601>'+ dt.getFullYear() + zeroPad(dt.getMonth()+1) + zeroPad(dt.getDate()) +
                        'T'+ zeroPad(dt.getHours()) +':'+ zeroPad(dt.getMinutes()) +':'+ zeroPad(dt.getSeconds()) +'</dateTime.iso8601>';
		...
		var parseXmlValue = function(node) {
			...
			for(var i=0; i < childs.length; i++) {
				...
				case "dateTime.iso8601":
                    ...
                    var m = jQuery(childs[i]).text().match(/^([0-9]{4})(-([0-9]{2})(-([0-9]{2})([T ]([0-9]{2}):([0-9]{2})(:([0-9]{2})(\.([0-9]+))?)?(Z|(([-+])([0-9]{2})(:)?([0-9]{2})))?)?)?)?$/);
                    if (!m) {
                        return null;
                    }
                    ...
                    fixDate(date, check);
                    if (m[14]) {
                        offset = Number(m[16]) * 60 + Number(m[18]);
                        offset *= m[15] == '-' ? 1 : -1;
                    }
